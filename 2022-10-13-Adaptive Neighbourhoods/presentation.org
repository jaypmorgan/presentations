...
So sparse amounts of information presents trust issues with our estimations of our
estimations of the class boundaries.
#+end_comment
...
create our neighbourhoods for which we wish to search for adversarial examples.
...
PGD can benefit from the adaptive neighbourhoods algorithm. Secondly, thanks for
adaptive neighbourhoods, we can meaningfully create adversarial examples for
non-image based data, enabling the use of adversarial examples for other tasks.
#+end_comment
